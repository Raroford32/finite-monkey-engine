...
- Orchestrator: coordinates planning, safety gates, fork validation
- SafetyGates: enforces defense-only policy (no mainnet writes, no exploit kits)
...
- `AGENT_MODE=defense-only` (default)
- `CHAIN_NETWORK` must not be `mainnet`, `livenet`, or `production`
- `CHAIN_RPC_URL` optional RPC for fork
